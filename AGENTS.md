...
- **Commit & PR Naming**: All commits and PR titles must follow Conventional Commits format. If a change spans multiple categories, the highest category in the list below must be used as the prefix (the "highest category wins" rule). Standard prefixes sorted from highest to lowest priority:
