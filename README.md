...
The integration is built with a **modular adapter pattern**, allowing community members to extend support for different controller models (like the P20B29, P23B32, P25B37, P25B85, and others) by defining their specific byte maps and command frame formats.
...
|---|---|---|---|---|
| **Joyonway P20B29** | PB555 (Segment) | 38400 8N1 | ✅ Supported | **Supported:** P20 family adapter. All read and write commands (independent single-speed pumps, blower, light with colors, ozone toggle, heater, setpoint, datetime schedules) are fully implemented. Manual mode switches (Manual Ozone / Manual Heating) are disabled due to an unresolved byte 13 encoding anomaly — Ozone and Heater switches are always available instead. *Note: Verified via unit tests. Any community live testing is highly appreciated!* |
| **Joyonway P23B32** | PB554 (Colour) | 38400 8N1 | ✅ Supported | **Supported:** P23 family adapter. All read and write commands (independent single-speed pumps, blower, light, heater, setpoint, ozone, datetime schedules) are fully implemented. *Note: Verified via unit tests. Any community live testing is highly appreciated!* |
| **Joyonway P25B37** | PB554 (Colour) | 38400 8N1 | ✅ Supported | **Supported:** Shares the P25 family broadcast layout, but utilizes discrete light controls and a variant command context byte. 80 % implemented and tested. |
...
- **Jets/pumps control** — supports speed percentage control (0% / 50% / 100%) for dual-speed pumps (e.g. P25B85) or standard ON/OFF controls for models with independent single-speed pumps (e.g. P20B29, P23B32)
...
| **[sergiu.balota](https://community.home-assistant.io/u/sergiu.balota)** | P20B29 broadcast signature variant (`0x06`) and heater state check fix |
| **[Yannickt26](https://community.home-assistant.io/u/yannickt26)** | Initial P20B29 control script and schedule unescaping analysis |
