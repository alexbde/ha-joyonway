...
- **Phase 35 (June 2026):** Implemented RS485 bus collision mitigation suggested by `@KDy` on the community forum by waiting for the idle sync frame (`b"\x1a\x01\x20\x08\x3c\xaa\x10\x00\x00\x6b\x73\xe4\xb9\x1d"`), sleeping for 30ms, and then writing command frames to hit the quiet bus window precisely. Completely removed the legacy `COMMAND_COOLDOWN` configuration and pacing logic from the coordinator and mock/live tests. Added a 2.0-second safety timeout on sync frame waits to prevent write locks/hanging queues. Refactored the live test suite (`test_spa_controls.py`) to align standalone test socket writes using the same sync helper. Analyzed bridge sniffer logs using a custom analysis script, confirming KDy's sync frame is indeed the most frequent non-broadcast frame on the bus (12.4% of non-broadcasts), with zero invalid frames detected across 62,100+ processed frames. All 140 unit tests and 64 dry-run simulation tests pass successfully.
- **Phase 36 (June 2026):** Refactored entities' classification categories: moved the bridge connectivity binary sensor under `EntityCategory.DIAGNOSTIC` and enabled it by default; grouped all schedule slot start/end times and slot enabled switches under `EntityCategory.CONFIG`. Renamed the German translations for schedule slot start times from "Start" to "Beginn" to ensure correct alphabetical sorting with "Ende". Verified that manual sync clock button is fully removed from codebase (remaining registry references in HA are due to local storage entity caching). All 140 unit tests and 64 dry-run simulation tests pass.
