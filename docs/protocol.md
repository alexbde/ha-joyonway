...
| **Heater Enabled (Armed)** | [❓] | Byte 14, bit `0x10` (states `0x50`/`0x54`/`0xD0`/`0xD4`) [✅] | Byte 14, bit `0x10` (states `0x50`/`0x54`/`0xD0`/`0xD4`) [✅]. P25B37: states `0x10`/`0x14` [✅], states `0x11`/`0x15` [✨] | Heater thermostat is armed/enabled. Note: Bit `0x10` is NOT set when ozone state is active. P20: no heating captures available. P25B37: base offset `0x00` means standby/heating state codes differ from P25B85 (standby state `0x10` and active heating state `0x14` confirmed via captures). |
...
| **Ozone Active (Auto)** | Byte 14, bit `0x01` [✅] | Byte 14: state `0x41` [✨] | Byte 14: state `0x41` [✅]. P25B37: state `0x01` [✨] | P20: bit-level check confirmed in production script; full-byte state values pending. P23/P25: state machine value. P25B37 uses base offset `0x00`, hence state `0x01`. |
| **Ozone Active (Manual)** | [❓] | Byte 14: state `0xC1` [✨] | Byte 14: state `0xC1` [✅]. P25B37: state `0x81` [✨] | Auto vs manual ozone state distinction unknown for P20. P25B37 uses base offset `0x00`, hence state `0x81`. |
...
*   **Hardware configuration and LCD UI profiles (P25):** The touchpad panel UI configures itself dynamically based on the controller board's status:
    *   **Blower availability** is broadcasted in **Byte 13, Bit 3 (`0x08`)** (`1` = Blower Present/Yes on P25B85, `0` = No Blower on P25B37), matching the board's physical DIP switch settings.
