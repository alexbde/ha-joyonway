"""Pytest coverage for P20B29 adapter behavior."""
...
adapters_p20 = load_module("joyonway.adapters.p20", PKG_DIR / "adapters" / "p20.py")

unescape_frame = protocol.unescape_frame
pseudo_unescape = protocol.pseudo_unescape
P20B29Adapter = adapters_p20.P20B29Adapter
P20B29_SIGNATURE = adapters_p20.P20B29_SIGNATURE
...
# Create a mock broadcast frame for P20B29
MOCK_P20_RAW = bytes(
    [0x1A, 0xFF, 0x01, 0x3C, 0xD2, 0xB4, 0xFF, 0x08, 0x01, 100, 0x00, 0x00]  # 0-11 (temp 100F -> 38C)
    + [0x14, 0x6F, 0x2C, 0x00, 104, 0x83, 0x00]  # 12-18 (jets 0x14, mode 0x6F, heater 0x2C, setpoint 104F -> 40C, light index 3 -> green)
    + [0x4A, 0x00, 0x0C, 0x1E, 0x56, 0x0F, 0x10, 0x2D, 0x00]  # 19-27 (heat schedules)
    + [
        0x00,
        0x48,
        0x00,
...
@pytest.fixture
def adapter() -> P20B29Adapter:
    return P20B29Adapter()
...
def test_adapter_properties(adapter: P20B29Adapter, logical_frame: bytes) -> None:
    ...
    assert logical_frame[: len(P20B29_SIGNATURE)] == P20B29_SIGNATURE


def test_parse_status_core_fields(adapter: P20B29Adapter, logical_frame: bytes) -> None:
...
def test_parse_rejects_wrong_signature(
    ...
    modified[8] = 0x02  # Change signature byte
    assert adapter.parse_status(bytes(modified)) is None


def test_parse_rejects_too_short_frame(adapter: P20B29Adapter) -> None:
    assert (
        adapter.parse_status(
            bytes([0x1A, 0xFF, 0x01, 0x3C, 0xD2, 0xB4, 0xFF, 0x08, 0x01])
        )
        is None
    )


def test_entity_descriptions(adapter: P20B29Adapter) -> None:
...
def test_parse_datetime(adapter: P20B29Adapter, logical_frame: bytes) -> None:
    result = adapter.parse_status(logical_frame)
...
def test_parse_schedules(adapter: P20B29Adapter, logical_frame: bytes) -> None:
    result = adapter.parse_status(logical_frame)
...
def test_build_light_command(adapter: P20B29Adapter) -> None:
    ...
    assert p_on[10] == 0x40
    assert p_on[12] == 0x04
    assert p_on[15] == 0x81  # Default ON (auto)

    off_frame = adapter.build_light_command(on=False)
    p_off = _frame_payload(off_frame)
    assert len(p_off) == 16
    assert p_off[15] == 0x80  # OFF

    color_frame = adapter.build_light_command(on=True, color="green")
    p_color = _frame_payload(color_frame)
    assert len(p_color) == 16
    assert p_color[15] == 0x83  # 0x80 + 3 (green)

    with pytest.raises(ValueError):
        adapter.build_light_command(on=True, color="invalid_color")


def test_build_jets_command(adapter: P20B29Adapter) -> None:
    # Left Pump ON
    on_left = adapter.build_jets_command("jets_left", "on")
    assert on_left is not None
    p_on_left = _frame_payload(on_left)
    assert len(p_on_left) == 16
...
def test_build_blower_command(adapter: P20B29Adapter) -> None:
...
def test_build_temp_command(adapter: P20B29Adapter) -> None:
...
def test_build_ozone_manual_command(adapter: P20B29Adapter) -> None:
    ...
    p_on = _frame_payload(on_frame)
    assert len(p_on) == 16
    assert p_on[7] == 0x80 and p_on[8] == 0x80

    off_frame = adapter.build_ozone_manual_command(on=False)
    p_off = _frame_payload(off_frame)
    assert len(p_off) == 16
    assert p_off[7] == 0x80 and p_off[8] == 0x00


def test_build_schedule_command(adapter: P20B29Adapter) -> None:
...
def test_build_datetime_command(adapter: P20B29Adapter) -> None:
